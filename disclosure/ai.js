...
- 진단명: 원문 진단명. 복사하다 글자 중간에 잘못 끼어든 공백만 붙이고("간 세포암종 의"→"간세포암종의") 단어 사이 띄어쓰기는 원문대로 둔다.
  원문에 코드가 있고 같은 코드로 앞에서 적었으면 비워도 된다. 원문에 코드가 없으면 모든 기록에 진단명을 반드시 적는다.
- 종류: 진단|입원|통원|수술|시술|투약|정기|재검 중 하나
  입원=입원 1건(수치=입원일수) / 통원=통원(수치=횟수·일수)
  투약=약 처방 기록("마지막처방일 … 30일이상", "N일치 처방" 포함, 수치=처방일수) / 정기=원문에 "정기처방", "계속 복용 중", "약 복용중"이라고 적힌 약
  수술=원문이 수술이라고 했거나 수술 섹션·수술명 칸에 적힌 것, "~술"로 끝나는 처치 전부(소작술·절제술·봉합술·해제술·삽입술 포함, 상세=수술명, 수치=횟수)
  시술=원문이 수술이라 하지 않은 주사·신경차단·물리치료·냉동치료·침술 등(상세=처치명)
  재검=추가검사·재검사 / 진단=그 밖의 진료·진단·날짜만 있는 기록
- 투약·정기의 상세에는 약 이름만 쓴다. 약 이름이 없으면 비운다(진단명이나 "투약"이라는 말을 쓰지 마라).
- 시작일·종료일: YYMMDD. 하루짜리는 시작일만. 연도가 없으면 MMDD("5월 28일"→0528).
  "5년전", "작년"처럼 정확한 날짜가 아니면 비운다(추측 금지).
- 수치: 숫자만("30일 이상"→30). 모르면 비운다.
- 칸은 항상 8칸이다. 빈 칸도 |로 남겨 칸 순서를 지킨다(코드를 모르면 줄번호 다음 칸을 비운다).
- "입원x", "입원 X", "입원없음"이면 입원 기록만 만들지 않는다. 같은 줄의 수술·진단 등 다른 기록은 그대로 적는다("수술x"도 같다).
- 입원이 여러 번이면 합치지 말고 한 줄씩. 똑같은 기록이 반복돼도 반복된 만큼 적는다.
- "ㄴ", "└", "→"로 시작하는 줄은 바로 위 병력의 치료 내용이다. 위 병력의 코드·진단명을 그대로 적는다.
  위에 병력 줄이 연달아 두 개 이상 있으면 각 병력마다 한 줄씩 적는다.
- 날짜 없이 "고혈압 / 당뇨 약 복용중"처럼 한 줄에 여러 병이 적힌 계속 복용 약은 병마다 정기 기록 한 줄씩 적는다(날짜는 비운다).
- 코드·진단명 없이 적힌 수술·처방·최근진료("간이식술", "간약 정기처방")는 원문 맥락상 연결되는 병력의 코드·진단명을 적는다.
  연결할 병력이 없으면 코드·진단명을 비운다.
- 고객 이름, 인사, 설계 요청, "입원/통원/처방", "3개월이내 병력" 같은 제목 줄은 기록이 아니다. 원문에 없는 내용은 절대 쓰지 마라.
...
15: 고혈압 / 당뇨 약 복용중
16: 2026-01-19 상세불명의 백내장,오른쪽
17: 2026-01-12 상세불명의 백내장,왼쪽
18: ㄴ인공수정체삽입술
출력:
...
15|~I10|고혈압|정기||||
15|~E11|당뇨병|정기||||
16|~H269|상세불명의 백내장,오른쪽|진단|260119|||
16|~H269|상세불명의 백내장,오른쪽|수술||||인공수정체삽입술
17|~H269|상세불명의 백내장,왼쪽|진단|260112|||
17|~H269|상세불명의 백내장,왼쪽|수술||||인공수정체삽입술
